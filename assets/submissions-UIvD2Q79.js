import{x as d}from"./index-DkoXGQQ4.js";import{i as p,p as f}from"./questionIdentifier-o3odOoR-.js";async function g(r){const s=r.trim(),e=f(s);if(e.kind==="uuid")return e.id;if(e.kind!=="composite")throw new Error(`题目标识不合法：${s}（请使用 UUID，或使用如 junzhuan_2026_03_12_05 的格式）`);const t=Number.parseInt(e.exerciseNoRaw,10),i=[];Number.isFinite(t)&&i.push(t),i.push(e.exerciseNoRaw),Number.isFinite(t)&&i.push(String(t));for(const n of i){const{data:o,error:u}=await d.from("essay_questions").select("id").eq("date_tag",e.dateTag).eq("exercise_no",n).limit(2);if(u)throw u;if(!o||o.length===0)continue;if(o.length>1)throw new Error(`题目不唯一：date_tag=${e.dateTag} exercise_no=${String(n)}（请检查数据重复）`);const _=o[0];if(!_?.id)throw new Error(`题目解析失败：date_tag=${e.dateTag} exercise_no=${String(n)} 未返回 id`);const a=String(_.id);if(!p(a))throw new Error(`题目表 essay_questions.id 必须是 UUID（当前查到: ${a}）。请确认数据库 schema：essay_questions.id 为 uuid（默认 gen_random_uuid()），不要把类似 ${s} 这种业务编码写进 id 列。`);return a}throw new Error(`找不到题目：date_tag=${e.dateTag} exercise_no=${e.exerciseNoRaw}`)}async function h(r){try{const s="sk-L8SP2yWjvfscsThsuydvlinxnzLTwmkLquEEkeWsescNhTvn",e="https://llm.xiaochisaas.com",t=await y(r),i=await fetch(`${e}/v1/chat/completions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${s}`},body:JSON.stringify({model:"claude-sonnet-4-6",messages:[{role:"user",content:[{type:"text",text:`你是一名高精度中文手写文本识别助手。请识别图片中的全部手写文字，并忠实转写原文。
...
请直接输出识别结果。`},{type:"image_url",image_url:{url:`data:image/jpeg;base64,${t}`}}]}],max_tokens:2e3,temperature:.1})});if(!i.ok){const u=await i.text();throw new Error(`LLM API 错误: ${u}`)}const o=(await i.json()).choices?.[0]?.message?.content?.trim();if(!o)throw new Error("未识别到文字");return{success:!0,text:o}}catch(s){return console.error("LLM OCR 错误:",s),{success:!1,error:s?.message||"OCR 识别失败"}}}async function y(r){return new Promise((s,e)=>{const t=new Image,i=new FileReader;i.onload=n=>{t.src=n.target?.result},t.onload=()=>{const n=document.createElement("canvas");let o=t.width,u=t.height;const _=2048;(o>_||u>_)&&(o>u?(u=u/o*_,o=_):(o=o/u*_,u=_)),n.width=o,n.height=u;const a=n.getContext("2d");if(!a){e(new Error("无法创建 canvas context"));return}a.drawImage(t,0,0,o,u);const l=m=>{const w=n.toDataURL("image/jpeg",m).split(",")[1]||null;return w&&w.length<4*1024*1024*.75?w:null};let c=l(.8)||l(.6)||l(.4)||l(.2);if(c||(n.width=o*.7,n.height=u*.7,a.drawImage(t,0,0,n.width,n.height),c=l(.5)||""),!c){e(new Error("图片压缩后仍超过大小限制"));return}s(c)},t.onerror=()=>e(new Error("图片加载失败")),i.onerror=()=>e(new Error("文件读取失败")),i.readAsDataURL(r)})}const E={async performLLMOCRFromFile(r){return h(r)},async createSubmission(r,s){const e=await g(r);if(!p(e))throw new Error(`question_id 必须是 UUID（解析后得到: ${e}）。这通常表示你把业务编码写进了 essay_questions.id，或 essay_questions.id 不是 uuid 类型。`);if(!p(s))throw new Error(`userId 必须是 UUID（收到: ${s}）`);const{data:t,error:i}=await d.from("essay_submissions").insert({question_id:e,user_id:s,ocr_status:"pending",grade_status:"pending",final_user_text:""}).select().single();if(i)throw i;return t},async createSubmissionWithOCR(r,s,e){const t=await g(r);if(!p(t))throw new Error(`question_id 必须是 UUID（解析后得到: ${t}）。这通常表示你把业务编码写进了 essay_questions.id，或 essay_questions.id 不是 uuid 类型。`);if(!p(s))throw new Error(`userId 必须是 UUID（收到: ${s}）`);const{data:i,error:n}=await d.from("essay_submissions").insert({question_id:t,user_id:s,ocr_status:"completed",grade_status:"pending",final_user_text:""}).select().single();if(n)throw n;return i},async uploadImage(r,s){const e=r.name.split(".").pop(),i=`submissions/${`${s}.${e}`}`,{error:n}=await d.storage.from("essay-images").upload(i,r,{upsert:!0});if(n)throw n;const{error:o}=await d.from("essay_submissions").update({image_path:i}).eq("id",s);if(o)throw o;return i},async updateOCRText(r,s,e){const{error:t}=await d.from("essay_submissions").update({ocr_status:e}).eq("id",r);if(t)throw t},async updateFinalText(r,s){const{error:e}=await d.from("essay_submissions").update({final_user_text:s}).eq("id",r);if(e)throw e},async getSubmissionById(r){const{data:s,error:e}=await d.from("essay_submissions").select(`
        *,
...
- 不要输出任何 JSON 之外的内容`,o=await fetch("https://llm.xiaochisaas.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer sk-L8SP2yWjvfscsThsuydvlinxnzLTwmkLquEEkeWsescNhTvn"},body:JSON.stringify({model:"claude-opus-4-6-thinking",messages:[{role:"user",content:i}],temperature:.3})});if(!o.ok){const c=await o.text();throw new Error(`LLM API 错误: ${c}`)}const u=await o.json(),_=u.choices[0].message.content;let a;try{let c=_.trim();c.startsWith("```json")?c=c.replace(/^```json\s*/,"").replace(/\s*```$/,""):c.startsWith("```")&&(c=c.replace(/^```\s*/,"").replace(/\s*```$/,""));const m=c.match(/\{[\s\S]*\}/);if(m&&(c=m[0]),a=JSON.parse(c),typeof a.total_score!="number")throw new Error("缺少 total_score 字段")}catch(c){console.error("JSON 解析失败:",c),console.error("原始输出:",_);const m=_.substring(0,200);throw new Error(`AI 返回格式错误：${c instanceof Error?c.message:"无法解析 JSON"}。返回内容预览：${m}...`)}const{error:l}=await d.from("essay_grades").insert({submission_id:r,total_score:a.total_score,content_score:a.content_score,structure_score:a.structure_score,expression_score:a.expression_score,matched_points:a.matched_points,missed_points:a.missed_points,deduction_points:a.deduction_points,overall_feedback:a.overall_feedback,improvement_suggestions:a.improvement_suggestions,raw_model_output:u});if(l)throw l;return await d.from("essay_submissions").update({grade_status:"completed"}).eq("id",r),{success:!0,grade:a}}};export{E as s};
