  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-6 max-w-md">
      ...
      <div v-if="ocrProcessing" class="bg-white rounded-lg shadow-sm p-6 text-center">
        ...
        <p class="text-gray-500 text-sm mt-2">正在识别手写文字，请稍候</p>
      </div>
...
const handleUploadAndOCR = async () => {
  ...
  console.log('Starting OCR with:', { questionId, userId })
  ...
  try {
    // 使用大模型 API 进行 OCR 识别
    const ocrResult = await submissionApi.performLLMOCRFromFile(imageFile.value)
    console.log('LLM OCR result:', ocrResult)

    if (ocrResult.success && ocrResult.text) {
      const submission = await submissionApi.createSubmission(questionId, userId)
      try {
        localStorage.setItem(`ocr_text:${submission.id}`, ocrResult.text)
      } catch {
