...
如果完整结构仍可能过长，请改用这个更短的 JSON 结构输出，仍然只允许输出 JSON：
{
  "total_score": 0,
  "content_score": 0,
  "structure_score": 0,
  ...
  attempts = [
    ...
    },
    {
      "prompt": (
        ...
        + "每个数组最多 4 项，reason 尽量短，overall_feedback 不超过 100 字，"
    ...
    },
    {
      "prompt": (
        prompt
        + "\n\n上一轮仍然过长。请使用最短版本输出：不要写对象数组，"
        + "`matched_points`、`missed_points`、`deduction_points` 全部改成字符串数组，"
        + "每个数组最多 3 项，overall_feedback 不超过 80 字。"
        + MINI_PROMPT_SUFFIX
      ),
      "max_tokens": 900,
    },
...
def grade_submission(session_token: str, submission_id: str):
  with get_conn() as conn:
    ...
    if count >= limit:
      return {"success": False, "error": "兄弟不要内卷，刷题刷太多了！"}
